...
<Canvas>
  <Story name="Daily view">
    {() => {
      const [date, set] = useState(dayjs(new Date(2022, 2, 2)));
      return (
...
<Canvas>
  <Story name="Select a date">
    {() => {
      const [date, set] = useState(dayjs(new Date(2022, 2, 15)));
      return (
        <div className="flex flex-col space-y-4 p-4">
          <div>Please select a date below: {date && date.format('DD.MM.YYYY')}</div>
          <Calendar
      ...
      return (
        <div className="flex flex-col space-y-4 p-4">
          ...
          <Calendar
            ...
            onDefaultActionClick={() => set(counter + 1)}
          />
        </div>
      );
    }}
