...
describe('TextField component', () => {
  ...
  test('can handle input changes correctly', async () => {
    ...
    render(<TextField placeholder="foo" onChange={func} />);
  ...
  test('can handle onEnter correctly', async () => {
    ...
    render(<TextField placeholder="foo" onEnter={func} />);
    const input = screen.getByPlaceholderText('foo');
  ...
  test('does not trigger onEnter when disabled', async () => {
    ...
    render(<TextField disabled placeholder="foo" onEnter={func} />);
    const input = screen.getByPlaceholderText('foo');
    await userEvent.type(input, '{enter}');
  ...
  test('executes onBlur event handler correctly', async () => {
    const func = jest.fn();
    render(<TextField placeholder="foo" onBlur={func} />);
    const input = screen.getByPlaceholderText('foo');
    await userEvent.type(input, '{enter}');
    input.blur();
    expect(func.mock.calls.length).toBe(1);
  ...
  test('renders icons corresponding to validation correctly', () => {
    const { container: valid } = render(
      <TextField placeholder="foo" validation="valid" />
    );
    const { container: warning } = render(
      <TextField placeholder="foo" validation="warning" />
    );
    const { container: error } = render(
      <TextField placeholder="foo" validation="error" />
    );
    ...
    expect(validIcon?.classList).toContain(
      `text-toolbox-feedback-${validationToColor('valid')}`
    );
    ...
    expect(errorIcon?.classList).toContain(
      `text-toolbox-feedback-${validationToColor('error')}`
    );
  });
});
