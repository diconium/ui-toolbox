...
describe('Tooltip/Display component', () => {
  ...
  test('can render the side of the component correctly', () => {
    ...
    expect(containerTop.firstChild).toHaveClass('-translate-y-full');
    expect(containerBottom.firstChild).toHaveClass('translate-y-full');
    expect(containerLeft.firstChild).toHaveClass('-translate-x-full');
    expect(containerRight.firstChild).toHaveClass('translate-x-full');
  });
