...
export function Display({ tooltip, side = 'right', className = '' }: Props) {
  const template = classNames(BASE_TEMPLATE, className, [
    ...
    side.includes('top') && 'left-0 -translate-y-full -mt-3 -ml-6',
    side.includes('bottom') && 'left-0 translate-y-full -bottom-4 -mr-6,',
  ]);
  const arrow = classNames(ARROW_BASE_TEMPLATE, [
    ...
    side === 'top'
      && 'left-1/2 bottom-0 -mb-6 translate-x-1/2 -ml-4 -rotate-90',
    side === 'top|left'
      && 'left-1/4 bottom-0 -mb-6 translate-x-1/2 -ml-4 -rotate-90',
    side === 'top|right'
      && 'right-1/4 bottom-0 -mb-6 translate-x-1/2 -ml-4 -rotate-90',
    side === 'bottom' && 'left-1/2 top-0 -mt-6 translate-x-1/2 -ml-4 rotate-90',
    side === 'bottom|left'
      && 'left-1/4 top-0 -mt-6 translate-x-1/2 -ml-4 rotate-90',
    side === 'bottom|right'
      && 'right-1/4 top-0 -mt-6 translate-x-1/2 -ml-4 rotate-90',
  ]);
  ...
  return (
    <div className={template}>
      ...
      {isSimple && (
        <span className="text-sm text-toolbox-neutral">{tooltip}</span>
      )}
      {!isSimple && tooltip}
    </div>
  );
